...
class HummingbirdPipeline(
    ...
    def encode_custom(
        ...
        device = device or self._execution_device
        if use_origin:
            current_image_encoder = self.image_encoder_origin
        else:
            current_image_encoder = self.image_encoder
        # set lora scale so that monkey patched LoRA
        # function of text encoder can correctly access it
        if lora_scale is not None and isinstance(self, StableDiffusionXLLoraLoaderMixin):
            self._lora_scale = lora_scale
        ...
        dtype = next(current_image_encoder.parameters()).dtype
    ...
    def _get_add_time_ids(
        ...
        if isinstance(self.unet, DDP):
            passed_add_embed_dim = (
                self.unet.module.config.addition_time_embed_dim * len(add_time_ids) + text_encoder_projection_dim
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        (pooled_prompt_embeds, negative_pooled_prompt_embeds) = self.encode_image(
            image, device=device, num_images_per_prompt=1
        )
        # 4. Prepare timesteps
        ...
        add_text_embeds_origin = torch.cat([negative_pooled_prompt_embeds_origin, add_text_embeds_origin], dim=0)
        add_time_ids = torch.cat([negative_add_time_ids, add_time_ids], dim=0)

        prompt_embeds = prompt_embeds.to(device) # * self.prompt_scale.exp()
        ...
        timestep_cond = None
        if isinstance(self.unet, DDP):
            time_cond_proj_dim = self.unet.module.config.time_cond_proj_dim
        else:
            time_cond_proj_dim = self.unet.config.time_cond_proj_dim
        ...
        added_cond_kwargs_origin = {"text_embeds": add_text_embeds_origin, "time_ids": add_time_ids}
